...
두 프로젝트를 동시에 개발하는 경우, 변경 사항을 반영하기 위해 `lein install`을 실행하고 REPL을
재시작하는 일은 매우 번거롭습니다. Leiningen은 *checkout dependencies* (짧게 *checkouts*)
라고 부르는 기능을 제공합니다. 이 기능을 사용하려면 아래 처럼 프로젝트 디렉토리 아래 `checkouts`
디렉토리를 만들어 합니다.
...
그 다음에 필요한 프로젝트 디렉토리의 심볼릭 링크를 checkouts 디렉토리 아래 만듭니다. Leiningen이
심볼릭 링크가 걸려있는 프로젝트에 있는 `project.clj` 파일을 사용하기 때문에 심볼릭 링크 이름은 아무
이름이나 써도 괜찮습니다. 보통 심볼릭 링크 이름은 심볼릭 링크가 가리키는 프로젝트 디렉토리 이름과 같은
이름을 사용합니다.
...
`checkouts` 아래 위치한 라이브러리는 리파지토리에서 가져온 라이브러리 보다 우선합니다. 그렇다고 해서
프로젝트에 있는 `:dependencies`를 대체하는 것은 아니기 때문에 라이브러리를 적어줘야합니다. 만약
위와 같은 디렉토리 모습이라면 아래 처럼 `project.clj`에 라이브러리를 추가해줘야 합니다:
...
그리고 메이븐 groupid인 `com.megacorp`는 checkouts 동작에 아무 영향을 주지 않습니다.
`commons`는 물리적인 디렉토리 구성을 따르지 않기 때문에 groupid 아래 있지 않아도 됩니다. 그래서
`suchwow`와 `commons` 심볼릭 링크는 `checkouts` 아래 같은 레벨로 있어도 됩니다.

`:dependencies`에 라이브러리를 추가해도 `lein` 계속 `~/.m2` 나 clojar 같은 리파지토리에서
라이브러리를 찾으려고 합니다. 만약 `lein`에서 "Could not find artifact suchwow:jar:0.3.9"와
같은 에러 메시지가 난다면 `project.clj`와 `suchwow/project.clj`가 다른 버전을 쓰고 있는지
확인해봐야 합니다.

메인 프로젝트와 `suchwow`를 함께 작업하는 동안 로컬 메이븐 리포지토리에 오래된 버전이 있다고 해도
두 프로젝트 파일 버전이 충돌 할 수 있습니다. 이럴 때는 `suchwow` 디렉토리에서 `lein install`을
실행합니다. 그래서 `suchwow` 버전은 아래 *세 개가* 모두 같아야 합니다:
suchwow `project.clj`, 메인 프로젝트 `project.clj`, 메인 프로젝트가 사용하는 리포지토리.

그리고 checkout 프로젝트의 의존성을 변경하는 경우에는 `lein install`을 실행하고 REPL을 다시 실행해줘야
합니다. 그냥 소스만 변경된 경우에는 자동으로 반영 됩니다.

Checkouts은 프로젝트에 선택 사항이기 때문에 프로젝트를 작업하는 동안 같은 checkouts을 가지고
있지 않습니다. 그래서 프로젝트를 push 하거나 merge 하기 전에 checkouts 디렉토리가 제외되어 있는지
확인해야합니다.

checkouts을 사용할 때 `base`을 덮어 쓰지 않도록 주의해야 합니다. 실제로 `lein with-profile foo run`가
아닌 `lein with-profile +foo run`를 사용한다는 것을 의미합니다.
